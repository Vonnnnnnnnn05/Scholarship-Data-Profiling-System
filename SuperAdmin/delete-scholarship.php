...
// Check if scholarship has scholars
$stmt = $conn->prepare("SELECT COUNT(*) as count FROM scholars WHERE scholarship_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$count = $stmt->get_result()->fetch_assoc()['count'];

if ($count > 0) {
    echo json_encode(['success' => false, 'message' => "Cannot delete scholarship. It has $count enrolled scholar(s)."]);
    exit;
}
...
if ($stmt->execute()) {
    ...
    $action = "Deleted scholarship ID: $id";
