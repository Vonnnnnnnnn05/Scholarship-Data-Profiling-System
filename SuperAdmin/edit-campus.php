...
// Get campus ID
$campus_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id FROM campuses WHERE campus_name = ? AND id != ?");
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE campuses SET campus_name = ? WHERE id = ?");
        $stmt->bind_param("si", $campus_name, $campus_id);
        
        if ($stmt->execute()) {
            // Log audit
            $user_id = $_SESSION['user_id'];
            $action = "Updated campus: $campus_name (ID: $campus_id)";
            ...
            $errors[] = "Failed to update campus";
        }
    }
}
?>
...
<head>
    ...
    <title>Edit Campus - SKSU SDP</title>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-edit"></i> Edit Campus</h1>
