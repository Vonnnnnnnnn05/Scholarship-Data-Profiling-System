...
// Get scholar data - MUST BE ENCODED BY THIS USER
$stmt = $conn->prepare("SELECT * FROM scholars WHERE id = ? AND encoded_by = ?");
$stmt->bind_param("ii", $scholar_id, $_SESSION['user_id']);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // Update only if this scholar belongs to current user
        $stmt = $conn->prepare("UPDATE scholars SET first_name = ?, last_name = ?, middle_initial = ?, year_level = ?, course = ?, campus_id = ?, scholarship_id = ? WHERE id = ? AND encoded_by = ?");
        $stmt->bind_param("sssssiiii", $first_name, $last_name, $middle_initial, $year_level, $course, $campus_id, $scholarship_id, $scholar_id, $_SESSION['user_id']);
