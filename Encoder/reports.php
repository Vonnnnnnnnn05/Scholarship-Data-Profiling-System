...
// Build query conditions - FILTER BY ENCODED_BY
$where = [];
$params = [];
$types = '';
...
$campuses = $conn->query("SELECT DISTINCT c.id, c.campus_name FROM scholars s LEFT JOIN campuses c ON s.campus_id = c.id WHERE s.encoded_by = {$_SESSION['user_id']} ORDER BY c.campus_name");

// Get scholarships for filter
$scholarships = $conn->query("SELECT DISTINCT sc.id, sc.scholarship_name FROM scholars s LEFT JOIN scholarships sc ON s.scholarship_id = sc.id WHERE s.encoded_by = {$_SESSION['user_id']} ORDER BY sc.scholarship_name");
...
<head>
    ...
    <title>My Reports - SKSU SDP</title>
    ...
    <div class="main-content">
        ...
        <div class="card">
            <h2 style="margin-bottom: 20px; color: #202124;">
                <?php 
                    if ($report_type == 'summary') echo 'Summary Report - My Data';
                    elseif ($report_type == 'detailed') echo 'Detailed Scholar Report - My Data';
                    else echo 'Campus Report - My Data';
                ?>
            </h2>
            ...
            <div class="table-wrapper">
                <table>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="<?php echo $report_type == 'detailed' ? '6' : ($report_type == 'summary' ? '5' : '4'); ?>" style="text-align: center; padding: 30px;">
